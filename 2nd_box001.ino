...
// =====================================================
// Homing - الليمت سويتش يشتغل (يوقف الحركة) في هذه المرحلة فقط.
// الحركة العادية (تنفيذ waypoints) ما تتأثرش بالليمت سويتش خالص.
// اتجاه الهومنج لكل محور = defaultBlockJogDir (هو نفسه الاتجاه
// اللي بيدخل جوه الليمت، يعني هو اتجاه الرجوع للصفر الميكانيكي).
// =====================================================
bool homingActive = false;

enum HomingPhase : uint8_t {
  HOME_IDLE = 0,
  HOME_Z    = 1,   // Z يرجع لأول ما يوصل الليمت بتاعه
  HOME_XY   = 2,   // بعدها X و Y يرجعوا معاً لحد ما كل واحد يضرب الليمت بتاعه
  HOME_DONE = 3
};
...
void startHoming() {
  if (homingActive) return;   // هومنج شغال بالفعل
...
void startSequence() {
  if (homingActive) return;   // ما تبدأش تسلسل وأنت لسه بتعمل هومنج
...
<div class="card">
  ...
  <div class="row">
    <button class="home" id="homeBtn">هومنج (رجوع للصفر)</button>
  </div>
</div>
...
async function refreshStatus(){
  ...
  try{
    ...
    if(d.homing){
      stateEl.textContent = "جارٍ الهومنج (رجوع للصفر بالليميت سويتش)";
...
void loop() {
  ...
  if (homingActive) {
    // في الهومنج فقط: الليمت سويتش هو اللي بيوقف كل محور.
    updateHoming(now);
  } else {
    // تحريك أي محور مسلَّح للحركة التلقائية (autoMoving)
    // ملاحظة: الليمت سويتش هنا لا يوقف ولا يمنع الحركة إطلاقاً،
    // هو بيشتغل وقت الهومنج فقط (updateHoming أعلاه).
    for (int i = 0; i < 3; i++) {
      Axis &ax = axes[i];
